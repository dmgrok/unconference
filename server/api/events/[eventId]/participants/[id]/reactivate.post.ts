...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Find the membership to reactivate
    ...
    memberships[membershipIndex].reactivatedAt = new Date().toISOString()
    memberships[membershipIndex].updatedAt = new Date().toISOString()
    
    // Remove suspension timestamp
    delete memberships[membershipIndex].suspendedAt
    ...
    logger.error('Error reactivating participant:', error)
    
    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to reactivate participant'
    })
  }
})

...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Find the membership to suspend
    ...
    memberships[membershipIndex].status = 'suspended'
    memberships[membershipIndex].suspendedAt = new Date().toISOString()
    ...
    logger.error('Error suspending participant:', error)
    
    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to suspend participant'
    })
  }
})

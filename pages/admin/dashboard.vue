  ...
  <v-container class="pa-6">
    <div class="d-flex justify-space-between align-center mb-6">
      ...
      <div class="d-flex gap-2">
        ...
        <v-btn color="secondary" prepend-icon="mdi-calendar-multiple" to="/admin/events">
          All Events
        </v-btn>
        <v-btn color="secondary" prepend-icon="mdi-account-group" to="/admin/users">
          User Management
        </v-btn>
        <v-btn color="secondary" prepend-icon="mdi-cog" to="/admin/settings">
          Platform Settings
        </v-btn>
      </div>
    </div>
      ...
      <v-col cols="12" md="6">
        <v-card>
          <v-card-title class="d-flex align-center justify-space-between">
            ...
            <v-btn size="small" color="primary" to="/admin/events">
              View All
            </v-btn>
          </v-card-title>
          <v-card-text>
...
async function exportMonitoringData() {
  try {
    const response = await $fetch('/api/admin/export-monitoring', {
      method: 'POST'
    }) as any
...
async function suspendEvent(eventId: string) {
  ...
  try {
    await $fetch(`/api/admin/events/${eventId}/suspend`, {
      method: 'POST'
    })
...
async function activateEvent(eventId: string) {
  try {
    await $fetch(`/api/admin/events/${eventId}/activate`, {
      method: 'POST'
    })
    await loadDashboardData()
  } catch (error) {
